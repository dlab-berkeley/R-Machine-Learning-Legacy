...
What do you notice about the new coefficients?
...
4.  Each hyperparameter's performance is summarized by the average
    ...
    hyperparameter that had the best average performance.
5.  We can then refit a new model to the entire training set, using our
    chosen hyperparameter. That's our final model - evaluate it on the
    test set!
...
1.  Perform hyperparameter tuning on the mixture value, and
...
Our comparison reveals that we can fit better models via
cross-validation than with just a single training set. This is reflected
by the lower RMSE and higher $R^2$ averages.
