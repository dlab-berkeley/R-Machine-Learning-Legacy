...
challenge7 <- read_csv(here("data/challenge7.csv"))
...
bayes_recipe <- recipe(Y ~ ., data = data_train)
...
rf_recipe <- recipe(Y~., data = data_train)%>%
  step_normalize(all_predictors())
...
Here's how to fit SVMs in `tidymodels`
...
svm_recipe <- recipe(Y~., data = data_train)%>%
  step_normalize(all_predictors())
...
## Running many different specifications at once 

Finally, let's return to a situation in which we have many models and want to compare all of them at once. A great feature of `tidymodels` are workflow sets. These allow us to compare and fit many model specifications and multiple recipes at once within a workflow (See Kuhn and Silge 2021). 


For this example, we will focus on a classification problem.

INSERT PROBLEM DESCRIPTION

We can use `last_fit()` to select the best model and final tuning parameters and fit on the entire training set. We then verify performance on our test data. Here is the pseudo code structure in `tidymodels`.
...
## See the confusion matrix for our predictions
best_fit %>% 
  collect_predictions() %>% 
...
Using the same dataset as Challenge 7, fit at least three different models working through the process we have covered in this workshop. 
