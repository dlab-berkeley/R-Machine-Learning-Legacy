...
We can, and in a real problem would, run our models on the entire dataset. However, doing so would be computationally expensive and take a lot of time. Thus for the purpose of the workshop, we sample to just 1/50 of the total data to demonstrate the ideas.
...
Now we define two different models, a random forest and a logistic regression model. 
...
It is also important to note that in production the grid process would be iterative. You may want to fit and evaluate more parts of the grid. 
...
In this model, the random forest coefficients take up most of the weight.
...
Finally, let's compute the accuracy for the model using the F-Score. The F-Score or F-measure is calculated from the precision and recall. We will also evaluate accuracy.  
